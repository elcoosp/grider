...
pub trait GridLike {
    ...
    // Helper function to calculate the minimum length of elements in an iterator.
    fn min_length<'a, T: Len + 'a>(iter: impl Iterator<Item = &'a T>) -> Option<usize> {
        iter.map(|item| item.len()).min()
    }
    ...
    fn filter_smallest_rows_with_tolerance(
        ...
        let min_length = Self::min_length(self.rows_iter()).unwrap_or(0);
    ...
    fn filter_smallest_columns_with_tolerance(
        ...
        let min_length = Self::min_length(self.columns_iter()).unwrap_or(0);
        let max_length = min_length as f64 * (1.0 + tolerance_percent / 100.0);
